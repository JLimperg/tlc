...
Lemma Acc_clos_trans : forall A (R:binary A) x,
  Acc R x -> Acc (tclosure' R) x.
Proof.
  induction 1 as [x0 _ H1]. apply Acc_intro. introv H2.
...
Lemma Acc_inv_trans : forall A (R:binary A) x y,
  tclosure' R y x -> Acc R x -> Acc R y.
Proof using.
...
Lemma tclosure'_wf : forall A (R:binary A),
  wf R -> wf (tclosure' R).
Proof using.
  unfold wf; unfold well_founded; auto with sets.
Qed.

Lemma tclosure_wf : forall A (R:binary A),
  wf R -> wf (tclosure R).
Proof using.
  introv H. eapply wf_iff.
  - apply tclosure'_wf; eassumption.
  - symmetry. apply tclosure_tclosure'.
Qed.

End Wf_Transitive_Closure.

Hint Resolve tclosure_wf tclosure'_wf : wf.
